...
function processActionFromMater(includeRegex, source, output, verbose, babelConfig, // eslint-disable-line flowtype/no-weak-types
data) {
  ...
  switch (data.type) {
    ...
    case _actions.TRANSFORM_FILE:
      return transformFile(includeRegex, source, output, data.filePath, verbose, babelConfig);
  ...
  return getFileContents(filePath).then(function (contents) {
    return new Promise(function (resolve, reject) {
      (0, _fs.stat)(filePath, function (err1, stats) {
        ...
        var result = (0, _babelCore.transform)(contents, Object.assign({
          filename: filePath
        }, babelConfig));
...
function _default(argv) {
  ...
  getBabelConfig(target, function (babelConfig) {
    process.on('message', processActionFromMater.bind(null, includeRegex, source, output, verbose, babelConfig));
  });
}